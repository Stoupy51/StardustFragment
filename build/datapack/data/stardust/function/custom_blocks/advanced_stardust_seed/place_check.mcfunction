...
#> stardust:custom_blocks/advanced_stardust_seed/place_check
#
...
execute if block ~ ~ ~ air run return run function stardust:custom_blocks/advanced_stardust_seed/place_main

# If not air, give back the item to the player
tag @e[type=item] add stardust.temp
execute as @p[tag=stardust.to_refund] at @s run loot spawn ~ ~ ~ loot stardust:i/advanced_stardust_seed
