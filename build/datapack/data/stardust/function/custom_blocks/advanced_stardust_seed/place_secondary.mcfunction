...
#
# @within	stardust:custom_blocks/advanced_stardust_seed/place_main [ as @n[tag=stardust.new] & at @s ]
#
...
tag @s add stardust.advanced_stardust_seed
tag @s add stardust.vanilla.minecraft_item_frame

# Add a custom name
data merge entity @s {"CustomName": {"translate": "stardust.advanced_stardust_seed"}}

# Modify item frame entity to match the custom block
item replace entity @s contents with minecraft:furnace[item_model="stardust:advanced_stardust_seed",custom_data={stardust:{item_frame_destroy:true,alt_destroy:"stardust.advanced_stardust_seed"}}]
