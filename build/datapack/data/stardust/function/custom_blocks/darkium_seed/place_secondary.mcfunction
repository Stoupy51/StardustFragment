...
#
# @within	stardust:custom_blocks/darkium_seed/place_main [ as @n[tag=stardust.new] & at @s ]
#
...
# Add a custom name
data merge entity @s {"CustomName": {"translate": "stardust.darkium_seed"}}

# Modify item frame entity to match the custom block
item replace entity @s contents with minecraft:furnace[item_model="stardust:darkium_seed",custom_data={stardust:{item_frame_destroy:true,alt_destroy:"stardust.darkium_seed"}}]
