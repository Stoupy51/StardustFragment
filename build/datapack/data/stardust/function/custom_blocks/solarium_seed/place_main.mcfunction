...
#> stardust:custom_blocks/solarium_seed/place_main
#
# @executed	as @e[type=item_frame,tag=...] & at @s
#
# @within	stardust:custom_blocks/solarium_seed/place_check
#

# Get the facing direction of the item frame
scoreboard players set #item_frame_facing stardust.data 1
execute if entity @s[type=item_frame] run function stardust:custom_blocks/solarium_seed/get_facing
...
execute as @n[tag=stardust.new] at @s run function stardust:custom_blocks/solarium_seed/place_secondary
...
scoreboard players add #total_solarium_seed stardust.data 1
