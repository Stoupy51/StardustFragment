...
#
# @within	stardust:custom_blocks/solarium_seed/place_main [ as @n[tag=stardust.new] & at @s ]
#
...
# Add a custom name
data merge entity @s {"CustomName": {"translate": "stardust.solarium_seed"}}

# Modify item frame entity to match the custom block
item replace entity @s contents with minecraft:furnace[item_model="stardust:solarium_seed",custom_data={stardust:{item_frame_destroy:true,alt_destroy:"stardust.solarium_seed"}}]
