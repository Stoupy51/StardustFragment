...
#> stardust:custom_blocks/stardust_furnace_generator/second
#
...
# Stop if full energy
execute if score @s energy.storage >= @s energy.max_storage run return run function stardust:custom_blocks/stardust_furnace_generator/stop
...
execute if score #burn_time stardust.data matches 0 run data modify entity @s item.components."minecraft:item_model" set value "stardust:stardust_furnace_generator"
...
execute if score #burn_time stardust.data matches 1.. run data modify entity @s item.components."minecraft:item_model" set value "stardust:stardust_furnace_generator_on"

# Update the gui & produce Energy while working
execute if score #burn_time stardust.data matches 1.. run scoreboard players add @s energy.storage 40
execute if score #burn_time stardust.data matches 1.. run playsound stardust:stardust_furnace_generator block @a[distance=..12] ~ ~ ~ 0.25
