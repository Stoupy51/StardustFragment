...
# @executed	at @s
#
# @within	stardust:custom_blocks/stardust_solar_panel/place_main [ at @s ]
#
...
# Add a custom name
data merge entity @s {"CustomName": {"translate": "stardust_solar_panel"}}

# Modify item display entity to match the custom block
item replace entity @s contents with minecraft:furnace[item_model="stardust:stardust_solar_panel"]
data modify entity @s transformation.scale set value [1.002f,1.002f,1.002f]
data modify entity @s brightness set value {block:15,sky:15}

# Energy part
tag @s add energy.send
scoreboard players set @s stardust.energy_rate 8
