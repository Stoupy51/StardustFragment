...
#> stardust:v4.0.7/load/enumerate
#
...
execute if score #stardust.major load.status matches 4 if score #stardust.minor load.status matches 0 unless score #stardust.patch load.status matches 7.. run scoreboard players set #stardust.patch load.status 7
