...
execute store success score #success stardust.data run data modify storage stardust:temp all set from entity @e[type=wither,tag=stardust.stardust_pillar,dx=0,dy=0,dz=0,predicate=stardust:equals_id,limit=1]
...
execute if score #success stardust.data matches 1 run data modify entity @s Rotation set from storage stardust:temp all.Rotation
