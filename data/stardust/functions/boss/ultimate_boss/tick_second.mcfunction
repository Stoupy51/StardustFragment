...
#Projectiles
execute at @e[tag=stardust.dragon] as @a[gamemode=!creative,gamemode=!spectator,distance=..200] run function stardust:boss/ultimate_boss/arrow
...
#Update states
execute as @e[tag=stardust.dragon] run scoreboard players add @e[tag=stardust.dragon,limit=1,sort=random] stardust.data 1
...
execute at @e[tag=stardust.ultimate_boss,sort=random] run bossbar set stardust:ultimate_boss players @a[distance=..500]
