...
#Generate Simplunium Ore Or More Stardust Ores (x8)
	...
	execute as @e[type=area_effect_cloud,tag=stardust.ore_pos,predicate=stardust:in_dim/cavern,limit=8] at @s run function stardust:calls/generate/simplunium
...
#Generate Azalea Tree Features
	...
	execute as @e[limit=8] run summon area_effect_cloud ~ 0 ~ {Tags:["stardust.ore_pos"],Duration:0}
	execute as @e[type=area_effect_cloud,tag=stardust.ore_pos,limit=8,predicate=stardust:in_dim/cavern] at @s run function stardust:calls/generate/feature/azalea_tree
