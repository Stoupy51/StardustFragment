...
#Generate Post Structures (Abandonned cave houses)
	...
	summon area_effect_cloud ~ 0 ~ {Tags:["stardust.struct_pos"],Duration:0}
	execute as @e[type=area_effect_cloud,tag=stardust.struct_pos,predicate=stardust:in_dim/overworld,limit=1] at @s run function stardust:calls/generate/structure/abandonned_cave_house
