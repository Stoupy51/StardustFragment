...
#Out of a Group
	...
	#Seeds
		...
		execute store success score #success stardust.data if entity @s[tag=stardust.seed] unless block ~ ~ ~ minecraft:moss_carpet run function stardust:destroy/seeds/all
		execute if score #success stardust.data matches 0 if entity @s[tag=stardust.diamond_seeds] unless block ~ ~-1 ~ minecraft:stone run function stardust:destroy/seeds/all
		execute if score #success stardust.data matches 0 if entity @s[tag=stardust.stardust_seeds] unless block ~ ~-1 ~ minecraft:diamond_block run function stardust:destroy/seeds/all
		execute if score #success stardust.data matches 0 if entity @s[tag=stardust.legendarium_seed] unless block ~ ~-1 ~ minecraft:lime_concrete run function stardust:destroy/seeds/all
		execute if score #success stardust.data matches 0 if entity @s[tag=stardust.solarium_seed] unless block ~ ~-1 ~ minecraft:magma_block run function stardust:destroy/seeds/all
		execute if score #success stardust.data matches 0 if entity @s[tag=stardust.darkium_seed] unless block ~ ~-1 ~ minecraft:obsidian run function stardust:destroy/seeds/all
