...
#SpawnItem
	...
	execute as @s[scores={SF_SCraft=4..}] run data modify entity @e[type=item,nbt={Item:{tag:{SF_Give:1s}}},limit=1,distance=..1] Item set from storage stardust:items 2013127

#Kill materials + Sound + Visual
	execute as @s[scores={SF_SCraft=4..}] run kill @e[type=item,nbt={Item:{Count:8b,tag:{SF_LegendaryFragment:1s}}},limit=1,sort=nearest,distance=..1]
