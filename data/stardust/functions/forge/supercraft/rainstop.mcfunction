...
execute at @e[type=item,nbt={Item:{Count:1b,tag:{AS:1b}}},limit=1,sort=nearest,distance=..1] at @e[type=item,nbt={Item:{id:"minecraft:lava_bucket",Count:1b}},limit=1,sort=nearest,distance=..1] run scoreboard players add @s SF_X 1
...
execute as @s[scores={SF_SCraft=5..}] run kill @e[type=item,nbt={Item:{id:"minecraft:lava_bucket",Count:1b}},limit=1,sort=nearest,distance=..1]
