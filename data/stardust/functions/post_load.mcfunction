...
execute unless score SimplEnergy load.status matches 1094..1100 run tellraw @a {"text":"Stardust Fragment Error: SimplEnergy v1.9.x is required, click [here] to download","italic":false,"color":"red","clickEvent":{"action":"open_url","value":"https://www.planetminecraft.com/data-pack/simplenergy/"}}
...
execute if score #game_version load.status matches 3105.. if score SimplEnergy load.status matches 1094..1100 run tellraw @a[tag=convention.debug] {"text":"[Loaded Stardust Fragment v3.4.4]","italic":false,"color":"green"}
