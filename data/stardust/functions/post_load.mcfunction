...
execute unless score #game_version load.status matches 2975.. run tellraw @a {"text":"Stardust Fragment Error: This version is only available with Minecraft 1.18.2+.","italic":false,"color":"red"}

execute if score #game_version load.status matches 2975.. if score SimplEnergy load.status matches 1090..1100 run tellraw @a[tag=convention.debug] {"text":"[Loaded Stardust Fragment v3.4.0 pre-release]","italic":false,"color":"green"}
