...
execute if score SimplEnergy_Missing StardustFragment_Data matches 1 run tellraw @a {"text":"Stardust Fragment Error: SimplEnergy v1.7.2+ is missing, click [here] to download","italic":false,"color":"red","clickEvent":{"action":"open_url","value":"https://www.planetminecraft.com/data-pack/simplenergy/"}}

execute unless score GameVersion load.status matches 2974.. run tellraw @a {"text":"Stardust Fragment Error: This version only available with Minecraft 1.18.2+.","italic":false,"color":"red"}
execute unless score SimplEnergy_Missing StardustFragment_Data matches 1 unless score SimplEnergy load.status matches 1072.. run tellraw @a {"text":"Stardust Fragment Error: SimplEnergy v1.7.2+ is required, click [here] to download","italic":false,"color":"red","clickEvent":{"action":"open_url","value":"https://www.planetminecraft.com/data-pack/simplenergy/"}}
execute if score GameVersion load.status matches 2974.. if score SimplEnergy load.status matches 1072.. run tellraw @a[tag=convention.debug] {"text":"[Loaded Stardust Fragment v3.3.2]","italic":false,"color":"green"}
