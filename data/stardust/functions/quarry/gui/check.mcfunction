...
execute unless data storage stardust:items Temp[{Slot:19b,tag:{SF_QuarryStart:1b}}] if score @s EF_kJ >= @s EF_kW run scoreboard players set @s SF_StatQ 1
execute unless data storage stardust:items Temp[{Slot:19b,tag:{SF_QuarryStart:1b}}] unless score @s EF_kJ >= @s EF_kW run tellraw @a[distance=..7] [{"text":"Not enough energy","italic":false,"color":"red"}]
