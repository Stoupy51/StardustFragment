...
data modify entity @e[type=item,limit=1,distance=..1,sort=nearest,tag=stardust.drop] Item set from storage stardust:main all.Drop
kill @e[type=item,nbt={Item:{tag:{simplenergy:{texture_item:1b}}}},limit=1,distance=..1,sort=nearest]
data remove storage stardust:main all.Drop
