...
execute unless score WormholePotionDisabled stardust.config matches 1 if entity @s[nbt={Inventory:[{tag:{stardust:{wormhole_potion:1b}}}]}] at @a if score @s stardust.wormhole = @p stardust.id run function stardust:right_click/wormhole_potion/teleport
