...
#Check what Sniper is it and set Base Damage
	...
	execute if score #sniper_type stardust.data matches 0 store result score #sniper_type stardust.data if data storage stardust:items SelectedItemTag.stardust.stardust_sniper run summon area_effect_cloud ~ ~ ~ {Tags:[stardust.to_throw,stardust.bullet,stardust.shooter,stardust.stardust_sniper],Age:0,Duration:0,Radius:0f}
	execute if score #sniper_type stardust.data matches 1 run scoreboard players set #sniper_type stardust.data -1
	execute if score #sniper_type stardust.data matches 0 store result score #sniper_type stardust.data if data storage stardust:items SelectedItemTag.stardust.awakened_stardust_sniper run summon area_effect_cloud ~ ~ ~ {Tags:[stardust.to_throw,stardust.bullet,stardust.shooter,stardust.awakened_stardust_sniper],Age:0,Duration:0,Radius:0f}
	execute if score #sniper_type stardust.data matches 1 run scoreboard players set #sniper_type stardust.data -2
	execute if score #sniper_type stardust.data matches 0 store result score #sniper_type stardust.data if data storage stardust:items SelectedItemTag.stardust.ultimate_sniper run summon area_effect_cloud ~ ~ ~ {Tags:[stardust.to_throw,stardust.bullet,stardust.shooter,stardust.ultimate_sniper],Age:0,Duration:0,Radius:0f}
...
#Launch the bullet
	...
	execute unless score #bullet_type stardust.data matches 1.. run playsound minecraft:block.note_block.snare ambient @s ~ ~ ~ 1 0.5
	...
	execute if score #bullet_type stardust.data matches 1.. as @e[type=area_effect_cloud,tag=stardust.to_throw,limit=1] at @s run function stardust:rightclick/snipers/projectile_move
