...
tp @s ^ ^ ^0.5
execute as @e[dx=0,dy=3,dz=0,distance=..3,tag=!StardustFragment_Shooter,tag=!StardustFragment_Shooted,tag=!global.ignore,tag=!global.ignore.kill,type=!player,type=!item,type=!#energy:valid_block_entities,nbt=!{Invulnerable:1b}] at @s run function stardust:rightclick/snipers/damage
