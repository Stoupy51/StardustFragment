...
#Update the gui & produce Energy while working
	...
	execute if score #temp stardust.data matches 1.. run playsound simplenergy:furnace_generator block @a[distance=2..12,tag=!simplenergy.ps.furnace_generator] ~ ~ ~ 0.04
	execute if score #temp stardust.data matches 1.. run playsound simplenergy:furnace_generator block @a[distance=..2,tag=!simplenergy.ps.furnace_generator] ~ ~ ~ 0.20
